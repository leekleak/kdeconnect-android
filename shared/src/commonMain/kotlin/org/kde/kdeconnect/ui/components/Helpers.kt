...
import org.kde.kdeconnect.generated.resources.device_list_empty
import org.kde.kdeconnect.generated.resources.on_non_trusted_message
import org.kde.kdeconnect.generated.resources.sentiment_sad
import org.kde.kdeconnect.generated.resources.verified_off
import org.kde.kdeconnect.generated.resources.wifi_off
    ...
    Box(                                                       // This scrollable box container is required
        ...
        if (!wifiAvailable) {
            BigPlaceholder(
                ...
                painter = painterResource(Res.drawable.wifi_off),
                text = stringResource(Res.string.on_non_trusted_message),
            )
        } else if (!trustedNetwork) {
            BigPlaceholder(
                modifier = Modifier.align(Alignment.Center),
                painter = painterResource(Res.drawable.verified_off),
                text = stringResource(Res.string.on_non_trusted_message),
            )
        } else {
            BigPlaceholder(
                modifier = Modifier.align(Alignment.Center),
                painter = painterResource(Res.drawable.sentiment_sad),
                text = stringResource(Res.string.device_list_empty)
            )
        }
    }
}
...
fun BigPlaceholder(modifier: Modifier = Modifier, painter: Painter, text: String) {
    val font = googleSans(weight = 600f, roundness = 100f)
    Column(
