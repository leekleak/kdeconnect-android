...
class ClipboardPlugin(context: Context, device: Device) : Plugin(context, device) {
    ...
    internal suspend fun userInitiatedSendClipboard() {
        ...
        if (clipboardManager!!.hasPrimaryClip()) {
            ...
            val text = org.jetbrains.compose.resources.getString(Res.string.pref_plugin_clipboard_sent)
            withContext(Dispatchers.Main) {
                Toast.makeText(this@ClipboardPlugin.context, text, Toast.LENGTH_SHORT).show()
            }
        }
    }
