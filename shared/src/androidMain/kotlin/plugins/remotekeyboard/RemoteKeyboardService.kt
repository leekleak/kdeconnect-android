        ...
        when (primaryCode) {
                ...
                try {
                    if (instances.size == 1) {  // single instance of RemoteKeyboardPlugin -> access its settings
                        ...
                        if (instances.isEmpty()) Toast.makeText(
                            this,
                            runBlocking { getString(Res.string.remotekeyboard_not_connected) },
                            Toast.LENGTH_SHORT
                        ).show()
                        else  // instances.size() > 1
                            Toast.makeText(
                                this,
                                runBlocking { getString(Res.string.remotekeyboard_multiple_connections) },
                                Toast.LENGTH_SHORT
                            ).show()
                    }
                } finally {
            ...
            KeyboardAction.CHECK_CONNECTION -> {
                if (isConnected) Toast.makeText(
                    this,
                    runBlocking { getString(Res.string.remotekeyboard_connected) },
                    Toast.LENGTH_SHORT
                ).show()
                else Toast.makeText(this, runBlocking { getString(Res.string.remotekeyboard_not_connected) }, Toast.LENGTH_SHORT).show()
            }
        }
    }
