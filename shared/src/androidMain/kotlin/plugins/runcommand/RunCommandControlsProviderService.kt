...
import org.kde.kdeconnect.generated.resources.tap_to_execute
...
class RunCommandControlsProviderService : ControlsProviderService() {
        ...
        return flow {
            getAllCommandsList().forEach { commandEntry ->
                emit(Control.StatelessBuilder(commandEntry.device.deviceId + ":" + commandEntry.command.key, getIntent(commandEntry.device))
                    ...
                    .setCustomIcon(Icon.createWithResource(this@RunCommandControlsProviderService, resources.getIdentifier("code", "drawable", packageName)))
                    .build())
            }
        }.asPublisher()
    }
            ...
            for (controlId in controlIds) {
                ...
                val control = if (commandEntry != null && commandEntry.device.isReachable) {
                    createStatefulBuilder(commandEntry, controlId)
                            ...
                            .setStatusText(runBlocking { getString(Res.string.tap_to_execute) })
                            .build()
    ...
    private fun createStatefulBuilder(commandEntry: CommandEntryWithDevice, controlId: String): Control.StatefulBuilder {
        return Control.StatefulBuilder(controlId, getIntent(commandEntry.device))
                ...
                .setCustomIcon(Icon.createWithResource(this, resources.getIdentifier("code", "drawable", packageName)))
    }

    private fun getIntent(device: Device?): PendingIntent {
        val intent = Intent(Intent.ACTION_MAIN)
            .setClassName(this, BuildConfig.MAIN_ACTIVITY_NAME)
            .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            .putExtra(KdeConnectKeyConstants.EXTRA_DEVICE_ID, device?.deviceId)

        if (device?.isReachable == true) {
            intent.putExtra(KdeConnectKeyConstants.EXTRA_PLUGIN_KEY, "RunCommandPlugin")
        }
